...
@Component({
  selector: "previsioAssistencia-popup",
  templateUrl: "previsioAssistencia.popup.html",
  styleUrls: ["./previsioAssistencia.popup.scss"],
})
      ...
      if (res == -1) {
        if (cas.assistencia.ConfirmacioTecnica) {
          this.presentarMissatgeError(
            "No es pot moure l'assistència del casteller, està confirmada tècnicament"
          );
          return;
        }
        if (this.tipusCerca == "assistents") {
