...
export class EsdevenimentEditPage extends PaginaNavegacio implements OnInit {
  ...
  private addGoogleMapScript() {
    ...
    return new Promise((resolve, reject) => {
      ...
      node.src = `https://maps.googleapis.com/maps/api/js?key=${environment.API_GOOGLE_KEY}&libraries=places&callback=initMap`;
  ...
  async prepararEsdeveniment(clone: boolean) {
    let esdeveniment: IEsdevenimentModel | undefined;
    ...
    if (esdeveniment == null) {
      ...
      esdeveniment = JSON.parse(JSON.stringify(esdeveniment));
      if (!esdeveniment) {
        console.error("error en el clonacion del objeto");
        this.presentarMissatgeError("Error en el clonado del objeto.");
        return;
      }
      esdeveniment.Id = "0";
    }
    ...
    this.formulari = this.fb.group(
      {
        ...
        DataIniCalendar: new FormControl(esdeveniment.DataIni),
        DataFiCalendar: new FormControl(esdeveniment.DataFi),
  ...
  changeDataFi() {
    if (
      ...
      this.formulari?.get("DataIniCalendar")?.setValue(this.formulari?.get("DataFiCalendar")?.value);
    }
  }
  changeDataInici() {
    if (
      ...
      this.formulari?.get("DataFiCalendar")?.setValue(this.formulari?.get("DataIniCalendar")?.value);
    }
  }
  get Id() {
  ...
  async onSubmit() {
    ...
    if (this.formulari?.valid) {
      ...
      this.formulari?.get("DataIni")?.setValue(dini);
      this.formulari?.get("DataFi")?.setValue(dFin);
