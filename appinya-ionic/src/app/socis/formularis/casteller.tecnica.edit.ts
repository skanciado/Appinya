...
export class CastellerTecEditPage extends PaginaNavegacio implements OnInit {
  ...
  async ngOnInit() {
    ...
    if (!this.casteller) {
      console.error("No s'ha pogut carregar el soci");
      this.presentarAlerta("Casteller no trobat", "No s'ha pogut carregar el soci")
      return;
    }
    if (this.casteller?.Posicions)
      this.casteller.Posicions.sort((a, b) =>
        a.Experiencia && b.Experiencia && a.Experiencia > b.Experiencia ? -1 : 1
      );
  ...
  async onSubmit() {
    ...
    if (!this.casteller) {
      console.error("No hay casteller assignat")
      return;
    }
  ...
  canviExperiencia(pos: IPosicioModel, event: any) {
    ...
    modal.onDidDismiss().then(async (detail: OverlayEventDetail) => {
      if (detail && detail.data) {
        ...
        this.casteller?.Posicions?.forEach((p1, index) => {
          let elem = detail.data.find((a: any) => {
            return a.IdPosicio === p1.IdPosicio;
          });
          if (!elem && this.casteller?.Posicions) {
            delete this.casteller?.Posicions[index];
          }
        });
        //Agregar
        detail.data.forEach((p1: any, index: any) => {
