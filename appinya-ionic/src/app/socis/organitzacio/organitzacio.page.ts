...
export class OrganitzacioPage extends PaginaNavegacio implements OnInit {
  ...
  async seleccion(org: IOrganitzacionModel, cas: ICastellerModel | null = null) {
    if (this.isAdmin() || this.isJunta()) {
      ...
      modal.onDidDismiss().then(async (detail: OverlayEventDetail) => {
        ...
        if (detail && detail.data) {
          ...
          if (cas != null) {
            let p1 = await lastValueFrom(this.organitzacioService
          ...
          let p2 = await lastValueFrom(this.organitzacioService
