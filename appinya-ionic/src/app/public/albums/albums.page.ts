...
export class AlbumsPage extends PaginaLlista implements OnInit {
   ...
   * */
  async canviarTemporada(): Promise<ITemporadaModel> {
  ...
  async canviarCerca() {
    ...
    if (this.tipusCerca == "actual") {
      ...
      this.iniciarLlista(fotos, this.actualitzarActual, null, 5);
    } else {
      // Si es historic
      if (this.temporada) {
        // Si te temporara pregunta al servidor
        this.iniciarLlista([], this.actualitzarHistoric, null, 5);
      } else {
