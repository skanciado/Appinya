...
export class AlbumEditPage extends PaginaNavegacio implements OnInit {
  ...
  canviarTitol(event) {
    this.Album.setValue(event.target.value);
  }
  get Id() {
  ...
  async assignarResponsable() {
    ...
    modal.onDidDismiss().then(async (detail: OverlayEventDetail) => {
      if (detail && detail.data) {
        this.formulari.get("Fotograf").value["Id"] = detail.data.Id;
        this.formulari.get("Fotograf").value["Alias"] = detail.data.Alias;
        this.formulari.get("Fotograf").value["Email"] = detail.data.Email;
        this.formulari.get("Fotograf").value["Foto"] =
  ...
  descargarFitxer(input) {
    if (input.files && input.files[0]) {
      ...
      reader.onload = () => {
        this.formulari.get("Portada").setValue(reader.result);
