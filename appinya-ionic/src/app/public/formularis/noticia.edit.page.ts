...
export class NoticiaEditPage extends PaginaNavegacio implements OnInit {
  ...
  async ngOnInit() {
    ...
    let noticia: INoticiaModel = null;
    if (this.id != "0") {
      noticia = await this.noticiesBs.obtenirNoticiaModel(this.id);
      if (noticia == null) {
    ...
    this.tipusNoticies = (await this.storeData.obtenirTipusNoticies()).filter(
      (tipus) => {
        if (
          ...
          return true;
        if (tipus.Id == "6" && this.isBar()) return true;
  ...
  async assignarResponsable() {
    ...
    modal.onDidDismiss().then(async (detail: OverlayEventDetail) => {
      if (detail && detail.data) {
        this.noticiaForm.get("UsuariReferencia").value["Id"] = detail.data.Id;
        this.noticiaForm.get("UsuariReferencia").value["Alias"] =
          detail.data.Alias;
        this.noticiaForm.get("UsuariReferencia").value["Email"] =
          detail.data.Email;
        this.noticiaForm.get("UsuariReferencia").value["Foto"] =
